        ...
        <div class="card-body">
            <form action="{{ route('admin.book.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for=""@error('name') class="text-danger" @enderror >Judul</label>
                    <input type="text" name="title" class="form-control @error('name') form-control is-invalid @enderror"
                    placeholder="Masukkan Judul Buku" value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="form-group @error('author_id') has-error @enderror">
                    ...
                    <select name="author_id" id="" class="form-control select2">
                        @foreach ($authors as $author)
                            <option value="{{ $author->id }}" class="">{{ $author->name }}</option>
                        @endforeach
                    </select>
                    @error('author_id')
                ...
                <div class="form-group">
                    ...
                    placeholder="Masukkan Jumlah Buku" value="{{ old('qty') }}">
                    @error('qty')
                        <span  class="text-danger"> {{ $message }} </span>
                    @enderror
                </div>
